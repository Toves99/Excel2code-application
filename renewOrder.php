...
$select="SELECT productId, productName,calcId,count,monthvalue,costperunit,totalCost,myapi,DATE_FORMAT(expiryDate, '%Y-%m-%d') AS formattedExpiryDate,productType,status FROM users,subscription where users.userId=subscription.userId AND users.username='$username'";
...
<head>
  ...
  <link href="assets/css/renew.css" rel="stylesheet">
...
<body>
  ...
  <section id="hero" class="d-flex align-items-center">
    ...
    <button class="btn5">My Order</button>
    </a>
    <a href="Select.php">
      <button class="btn6">Place New Order</button>
      </a>
      <button class="btn7">Renew Order</button>

      <button class="btn8">My API</button>
    </div>
...
if ($num > 0) {
    while ($data = mysqli_fetch_assoc($result)) {
        ...
        echo "<td>" . $data['calcId'] . "</td>";
        echo "<td>" . $data['count'] . "</td>";
        echo "<td>" . $data['monthvalue'] . "</td>";
        echo "<td>" . $data['totalCost'] . "</td>";
        echo "<td>" . $data['formattedExpiryDate'] . "</td>";
        echo "<td>
            <a href='viewMore.php?productId=" . $data['productId'] . "'>
              <button class='but1'>view more</button>
            </a>
        </td>";
        ...
        echo "<td>
            <a href='#'>
              <button class='but2'>renew</button>
            </a>
        </td>";
    ...
    echo "
        <tr data-href='course-login.php'>
          <td colspan='7' class='error-message'>You have not Uploaded any Excel Calculator.</td>
        </tr>
    ";
}
?>
    ...
    <h3>Click view more on the table to see more details  of the subscription</h3>
  </section><!-- End Hero -->
