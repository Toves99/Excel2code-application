...
if (isset($_POST['submit'])) {
 ...
 cb.userid =$user_id  AND cb.paymentstatus=0 ORDER BY cb.debitdatetime DESC limit 1";
  ...
  if($num_rows>0){
    if($resultcode!=0){
      echo "<script>alert('Your transaction payment was unsuccessiful we have cancelled your subscription please try again later');window.location.href = 'addExcel.php';</script>";
      $paymentstatus=2;
    }
    else{
      ...
      $sqlquery="INSERT INTO `orders`(`userId`,`servicename`, `count`,`service`,`validityperiod`,`expirydate`,`paidamount`,`paymentstatus`,`paymentmessage`,`serviceid`,`orderprice`,`costperunit`,`packagename`)
      VALUES ('$user_id', '$serviceName','$count','$individualService','$monthValue','$newUploadedTime','$paymentAmount','$paymentstatus','paid','$ServiceId','$totalCost','$costPerUnit','$packagename')";
  ...
  $update_result = mysqli_query($conn, $sqlupdatestatus);
   ...
   <script>
        ...
        setTimeout(function(){
            ...
            window.location.href = 'uploadExcelneworder.php';
        }, 7000); // Hide the div after 7 seconds and redirect to 'uploadExcelneworder.php'
...
<head>
  ...
  <link href="assets/css/verify4.css" rel="stylesheet">
...
<body>
 ...
 <form action="" method="POST">
   ...
   <p class="vp">Please click the button below to
    ...
    <h6><?php //echo "costperunit".$costPerUnit?></h6>
  </form>
